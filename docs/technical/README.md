...
| Güvenli SDLC | `03-Backend/src/veri_kalitesi/secure_sdlc/` | `RepositorySecretScanner`, `PythonDependencyInventoryBuilder`, `SastReleaseGate`, `DependencyVulnerabilityReleaseGate`, `PentestFindingTracker` | Yerel secret taraması, doğrudan bağımlılık SBOM'u, veri-minimum SAST/bağımlılık zafiyet kapıları ve pentest bulgu takibi |
...
| Güvenli SDLC | Kısmen uygulanmış | Secret scanner, direct SBOM, SAST/doğrudan bağımlılık zafiyet kapıları ve pentest bulgu takibi | Gerçek scanner/transitive SCA/DAST/pentest, CI zorlaması ve istisna akışı |
