...
| Güvenli SDLC | `03-Backend/src/veri_kalitesi/secure_sdlc/` | `RepositorySecretScanner`, `PythonDependencyInventoryBuilder`, `SastReleaseGate`, `DependencyVulnerabilityReleaseGate` | Yerel secret taraması, doğrudan bağımlılık SBOM'u ve veri-minimum SAST/bağımlılık zafiyet kapıları |
...
| Güvenli SDLC | Kısmen uygulanmış | Secret scanner, direct SBOM, SAST ve doğrudan bağımlılık zafiyet kapıları | Gerçek scanner/transitive SCA/DAST, CI zorlaması ve istisna akışı |
