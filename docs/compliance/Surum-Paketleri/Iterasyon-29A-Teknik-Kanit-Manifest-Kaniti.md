...
```bash
python3 -m pytest -q tests/unit/test_secure_sdlc_evidence.py
python3 -m pytest -q tests/unit/test_secure_sdlc*.py
python3 -m pytest -q
python3 -m ruff check .
python3 -m ruff format --check src/veri_kalitesi/secure_sdlc/evidence.py tests/unit/test_secure_sdlc_evidence.py
python3 -m mypy src/veri_kalitesi/secure_sdlc/evidence.py tests/unit/test_secure_sdlc_evidence.py
python3 -m compileall -q docs/backend/src tests
PYTHONPATH=docs/backend/src python3 -m veri_kalitesi.secure_sdlc .
cmp -s <(PYTHONPATH=docs/backend/src python3 -m veri_kalitesi.secure_sdlc.evidence docs/compliance/Surum-Paketleri/Iterasyon-29A-Teknik-Kanit-Katalogu.json .) docs/compliance/Surum-Paketleri/Iterasyon-29A-Teknik-Kanit-Manifesti.json
```
