...
def job_tables(schema: str = DEFAULT_SCHEMA_NAME) -> JobTables:
    ...
    jobs = Table(
        ...
        CheckConstraint(
            ...
            name="ck_background_jobs_blocked_consistency",
        ),
    )
    Index(
    ...
    workers = Table(
        ...
        Column("supported_job_types", JSON, nullable=False),
        ...
        CheckConstraint("capacity > 0", name="ck_workers_capacity"),
        CheckConstraint(
            "state IN ('STARTING', 'RUNNING', 'DRAINING', 'STOPPED', 'UNHEALTHY')",
            name="ck_workers_state",
        ),
        CheckConstraint("version >= 0", name="ck_workers_version"),
    )
    Index("ix_dq_workers_state_last_seen", workers.c.state, workers.c.last_seen_at)
    return JobTables(background_jobs=jobs, dead_letters=dead_letters, workers=workers)
...
class PostgreSQLJobQueueRepository:
                ...
                session.execute(
                    ...
                    .where(
                        or_(
                            ...
                            and_(
                                t.c.status == JobStatus.BLOCKED.value,
                                t.c.blocked_until <= claimed_at,
                            ),
                        )
                    )
                    .order_by(
    ...
    def register_worker(
        ...
        """Worker kaydını upsert et; mevcut kaydı RUNNING olarak güncelle."""

        w = self._tables.workers
        now = registration.last_seen_at
        with transactional_session(self._session_factory) as session:
            existing = (
                session.execute(select(w).where(w.c.worker_id == registration.worker_id))
                .mappings()
                .one_or_none()
            )
            if existing is not None:
                result = session.execute(
                    update(w)
                    .where(
                        w.c.worker_id == registration.worker_id,
                        w.c.version == existing["version"],
                    )
                    .values(
                        hostname=registration.hostname,
                        capacity=registration.capacity,
                        supported_job_types=list(registration.supported_job_types),
                        state=WorkerState.RUNNING.value,
                        last_seen_at=now,
                        stopped_at=None,
                        version=w.c.version + 1,
                    )
                )
                if cast(CursorResult[Any], result).rowcount == 0:
                    raise JobConcurrencyError("Worker registration version conflict.")
            else:
                session.execute(
                    insert(w).values(
        ...
        *,
        ...
        with transactional_session(self._session_factory) as session:
            result = session.execute(
                update(w)
                .where(
                    w.c.worker_id == worker_id,
                    w.c.version == expected_version,
                    w.c.state.in_((WorkerState.RUNNING.value, WorkerState.STARTING.value)),
                )
                .values(
            ...
            if cast(CursorResult[Any], result).rowcount == 0:
                current = (
                    session.execute(select(w).where(w.c.worker_id == worker_id))
                    .mappings()
                    .one_or_none()
                )
                if current is None:
                    raise JobNotFoundError("Worker not found.")
                raise JobConcurrencyError("Worker heartbeat version conflict.")
            updated = session.execute(select(w).where(w.c.worker_id == worker_id)).mappings().one()
        return _row_to_worker(updated)
        ...
        with transactional_session(self._session_factory) as session:
            result = session.execute(
                ...
                .where(
                    w.c.worker_id == worker_id,
                    w.c.version == expected_version,
                    w.c.state == WorkerState.RUNNING.value,
                )
                .values(
                    state=WorkerState.DRAINING.value,
                    last_seen_at=drain_at,
                    version=w.c.version + 1,
                )
            )
            if cast(CursorResult[Any], result).rowcount == 0:
                raise JobConcurrencyError("Worker drain version conflict.")
            updated = session.execute(select(w).where(w.c.worker_id == worker_id)).mappings().one()
        return _row_to_worker(updated)
        ...
        with transactional_session(self._session_factory) as session:
            result = session.execute(
                ...
                .where(
                    w.c.worker_id == worker_id,
                    w.c.version == expected_version,
                    w.c.state.in_((WorkerState.RUNNING.value, WorkerState.DRAINING.value)),
                )
                .values(
                    state=WorkerState.STOPPED.value,
            ...
            if cast(CursorResult[Any], result).rowcount == 0:
                raise JobConcurrencyError("Worker stop version conflict.")
            updated = session.execute(select(w).where(w.c.worker_id == worker_id)).mappings().one()
        return _row_to_worker(updated)
    ...
    def update_progress(
        ...
        *,
        ...
        if not isinstance(progress_percent, int) or not 0 <= progress_percent <= 100:
            raise JobValidationError("Progress percent must be between 0 and 100.")
        _validate_worker_id(worker_id)
        ...
        with transactional_session(self._session_factory) as session:
            ...
            if cast(CursorResult[Any], result).rowcount == 0:
                raise JobConcurrencyError("BackgroundJob progress version conflict.")
            updated = session.execute(select(t).where(t.c.job_id == job_id)).mappings().one()
        return _row_to_job(updated)
    ...
    def block_job(
        ...
        *,
        ...
        _validate_worker_id(worker_id)
        _validate_code("blocked_reason_code", reason_code)
        if blocked_until.tzinfo is None or blocked_until.utcoffset() is None:
            raise JobValidationError("blocked_until must be timezone-aware.")
        blocked_at = _aware_now(now)
        _validate_audit_pair(audit_event, audit_outbox)
        t = self._tables.background_jobs
        with transactional_session(self._session_factory) as session:
            result = session.execute(
            ...
            if cast(CursorResult[Any], result).rowcount == 0:
                raise JobConcurrencyError("BackgroundJob block version conflict.")
            if audit_event is not None and audit_outbox is not None:
                audit_outbox.stage(audit_event, session=session)
            updated = session.execute(select(t).where(t.c.job_id == job_id)).mappings().one()
        return _row_to_job(updated)
...
def _row_to_worker(row: RowMapping) -> WorkerRegistration:
    job_types = row["supported_job_types"]
