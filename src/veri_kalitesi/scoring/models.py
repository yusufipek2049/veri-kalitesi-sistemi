...
class ScorePublicationStatus(str, Enum):
    PUBLISHED = "PUBLISHED"
...
class QualityScore:
    ...
    included_component_count: int | None = None
    excluded_component_count: int | None = None
...
class ScorePublication:
    """Atomik yayın kaydı — PUBLISHED/SUPERSEDED state-machine."""
    ...
    def __post_init__(self) -> None:
        if self.status is ScorePublicationStatus.SUPERSEDED:
            if self.superseded_at is None:
                object.__setattr__(self, "superseded_at", self.published_at)
        elif self.superseded_at is not None:
            raise ValueError("PUBLISHED publication cannot have superseded_at.")
