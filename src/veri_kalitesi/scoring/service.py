...
class ScoringService:
    ...
    def calculate_enterprise_score(
        ...
        *,
        ...
        if _candidates is not None:
            ...
            candidates = [
                (scores_by_source[data_source_id], data_source_id)
                for data_source_id in data_source_ids
            ]
    ...
    def calculate_full_score_set(
        ...
        *,
        ...
        Returns rule, dataset, dimension, source and enterprise scores.
        The caller (publication service) is responsible for persistence.
        """
        ...
        decisions_by_rule_version = self._partial_decisions(execution, versions, None)
        ...
        for dimension in QualityDimension:
            dim_candidates = [
                (score, self.rule_catalog.get_version(score.rule_version_id))
                for score in rule_scores
                if score.rule_version_id is not None
                and self.rule_catalog.get_rule(
                    self.rule_catalog.get_version(score.rule_version_id).quality_rule_id
                ).primary_dimension
                is dimension
            ]
            if dim_candidates:
        ...
        if self.source_catalog is not None:
            source_datasets: dict[str, list[tuple[QualityScore, Dataset]]] = {}
            dataset_criticality: dict[str, Criticality] = {}
            for ds_score in dataset_scores:
                if ds_score.scope_id is None:
                    continue
                dataset = self.source_catalog.get_dataset(ds_score.scope_id)
            ...
            scores_by_source = {
                s.scope_id: s
                for s in all_scores
                if s.scope_type is ScoreScopeType.SOURCE and s.scope_id is not None
            }
            seen_sources: set[str] = set()
            enterprise_candidates: list[tuple[QualityScore, str]] = []
            for rv_id in execution.rule_version_ids:
                version = self.rule_catalog.get_version(rv_id)
                rule = self.rule_catalog.get_rule(version.quality_rule_id)
                dataset = self.source_catalog.get_dataset(rule.dataset_id)
                source_id = dataset.data_source_id
                if source_id not in seen_sources and source_id in scores_by_source:
    ...
    def reproduce_rule_score(
        ...
        *,
        ...
        """Deterministic single-rule score replay without persistence."""
        return self._score_rule(execution, version, result, configuration)
