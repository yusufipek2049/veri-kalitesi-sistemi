...
class SQLiteDataSourceRepository:
    ...
    def list_all_data_sources(self) -> list[DataSource]:
        rows = self.connection.execute(
            "SELECT * FROM data_sources ORDER BY name COLLATE NOCASE, data_source_id"
        ).fetchall()
        return [_row_to_data_source(row) for row in rows]
    ...
    # --- DS-04 stubs (not production path) ---
    ...
    def record_discovery_request(
        ...
        *,
        ...
        raise NotImplementedError("SQLite repository does not support async discovery.")
    ...
    def update_discovery_scope(
        ...
        *,
        ...
        raise NotImplementedError("SQLite repository does not support discovery scopes.")
    ...
    def persist_metadata_diff(
        ...
        *,
        ...
        raise NotImplementedError("SQLite repository does not support metadata diffs.")
    ...
    def _insert_metadata_discovery_result(self, result: MetadataDiscoveryResult) -> None:
        self.connection.execute(
            ...
            (
                ...
                result.error_class.value if result.error_class else None,
                result.message,
                json.dumps([metadata_change_to_dict(change) for change in result.changes]),
