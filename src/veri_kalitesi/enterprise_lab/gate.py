...
def verify_enterprise_lab_configuration(path: Path) -> EnterpriseLabEvidence:
    ...
    max_evidence_age = payload.get("max_evidence_age_seconds")
    if (
        isinstance(max_evidence_age, bool)
        or not isinstance(max_evidence_age, int)
        or max_evidence_age <= 0
        or max_evidence_age > 86400
    ):
        raise EnterpriseLabConfigurationError("LAB_EVIDENCE_AGE_INVALID")
    ...
    return EnterpriseLabEvidence(
        ...
        max_evidence_age_seconds=max_evidence_age,
        checks=(
