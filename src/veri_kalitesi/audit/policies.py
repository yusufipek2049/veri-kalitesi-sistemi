...
# Nesnenin okunabilir adi; tum aksiyonlar icin izin verilen gosterim
# metadata'sidir (kimlik cozulebilirlik icin). Hassas icerik kontrolleri
# yine de uygulanir.
_OBJECT_DISPLAY_FIELD = "object_name"


def build_default_redaction_policy() -> AuditRedactionPolicy:
    allowed_fields = _allowed_fields_by_action()
    return AuditRedactionPolicy(
        version="AUDIT_REDACTION_V4",
        allowed_fields_by_action={
            action: frozenset({*fields, _OBJECT_DISPLAY_FIELD})
            for action, fields in allowed_fields.items()
        },
    )


def _allowed_fields_by_action() -> dict[str, frozenset[str]]:
    return {
        ...
        "DATA_SOURCE_DEACTIVATED": frozenset({"data_source_revision", "policy_version", "status"}),
        ...
        "QUALITY_RULE_CREATED": frozenset({"rule_version_id", "version_no", "rule_type", "status"}),
        ...
        "NOTIFICATION_EVENT_PUBLISHED": frozenset(
            {
                "event_id",
                "event_type",
                "scope_type",
        ...
        "NOTIFICATION_DELIVERY_ATTEMPTED": frozenset(
            {
                "delivery_id",
                "event_id",
                "channel_id",
        ...
        "NOTIFICATION_DELIVERY_COMPLETED": frozenset(
            {
                "delivery_id",
                "event_id",
                "channel_id",
        ...
        "NOTIFICATION_DELIVERY_FAILED": frozenset(
            {
                "delivery_id",
                "event_id",
                "channel_id",
        ...
        "NOTIFICATION_DELIVERY_READ": frozenset(
            {
                "delivery_id",
                "event_id",
                "read_at",
        ...
        "NOTIFICATION_SUBSCRIPTION_CREATED": frozenset(
            {
                "subscription_id",
                "event_type",
                "channel_id",
        ...
        "NOTIFICATION_SUBSCRIPTION_UPDATED": frozenset(
            {
                "subscription_id",
                "event_type",
                "channel_id",
        ...
        "NOTIFICATION_CHANNEL_CONFIGURED": frozenset(
            {
                "channel_id",
                "channel_type",
                "status",
        ...
        "DATASET_PREVIEW_VIEWED": frozenset({"row_count", "limit"}),
        "EXECUTION_MANUAL_STARTED": frozenset({"rule_version_count", "execution_mode"}),
        "DATASET_UPDATED": frozenset({"changed_fields"}),
        "FIELD_UPDATED": frozenset({"changed_fields"}),
        "METADATA_DISCOVERY_REQUESTED": frozenset({"data_source_id"}),
        "SCHEDULE_ACTIVATED": frozenset({"is_active"}),
        "SCHEDULE_DEACTIVATED": frozenset({"is_active"}),
    }
