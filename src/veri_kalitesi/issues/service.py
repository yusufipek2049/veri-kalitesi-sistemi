...
class IssueService(Generic[AuditT]):
    ...
    def create_for_trigger(
        ...
        issue = DataQualityIssue(
            ...
            last_seen_at=now,
            title=trigger.title,
            source_execution_id=trigger.execution_id,
            source_rule_version_id=trigger.rule_version_id,
        )
        ...
        notification_batch = self._prepare_notification_batch(trigger, context)
        try:
        ...
        if self.notification_batch_stager is None:
            try:
                self._publish_notification(trigger, context, stored)
            except (IssueNotificationTechnicalError, IssueNotificationConfigurationError):
                pass
        return stored
    ...
    def create_manual(
        ...
        """Manuel issue olusturma — USER rol, scope ve idempotency dogrulamasi."""
        validate_manual_issue_draft(draft)
        context = self._authorize_actor(
            actor_context,
            self.access_policy.manual_creator_actor_types,
            "create manual issues",
        )
        now = self._now()
        assignment = self.assignment_resolver.resolve_assignment(
            IssueTrigger(
                trigger_type=IssueTriggerType.MANUAL,
                scope_type=draft.scope_type,
                scope_id=draft.scope_id,
                deduplication_key=draft.idempotency_key,
                occurred_at=now,
        ...
        dedup_key = (
            f"manual:{draft.creator_user_id}:{digest_idempotency_key(draft.idempotency_key)}"
        )
        dedup_digest = _digest_text(dedup_key)
        issue_id = str(uuid5(_ISSUE_NAMESPACE, dedup_digest))
        ...
        history = IssueHistoryEntry(
            ...
            source_event_payload_digest=dedup_digest,
        )
        ...
        try:
            stored = self.repository.add_or_increment(
                issue,
                history,
                payload_digest=dedup_digest,
                source_event_occurred_at=now,
                relationship=None,
                relationship_history=None,
                audit_event=prepared,
                reopen_audit_event=prepared,
            ...
            raise IssueTechnicalError(
                "Manual issue could not be persisted.", draft.correlation_id
            ) from exc
        self.transactional_audit.publish_pending()
        return stored
    ...
    def reassign(
        ...
        notification_batch = self._prepare_assignment_notification_batch(issue, history, context)
        try:
    ...
    def _prepare_notification_batch(
        ...
        """Prepare notification batch for pre-commit staging."""
        if self.notification_batch_stager is None:
            return None
        ...
        try:
            return self.notification_batch_stager.prepare_batch(
                event,
                recipient_user_ids=(),
    ...
    def _prepare_assignment_notification_batch(
        self,
        issue: DataQualityIssue,
        history: IssueHistoryEntry,
        context: ActorContext,
    ) -> object | None:
        """Prepare assignment notification batch for pre-commit staging."""
        if self.notification_batch_stager is None:
            return None
        event = NotificationEvent(
        ...
        recipient_ids = (issue.assignee_user_id,) if issue.assignee_user_id else ()
        try:
            return self.notification_batch_stager.prepare_batch(
                event,
