...
class ExecutionStrategyEngine:
    ...
    - PARTITION resume yalnız tamamlanmış checkpoint sınırından devam eder;
      tamamlanmış sınır yoksa reddedilir (fail-closed).
    ...
    def resolve(
        ...
        *,
        ...
        # PARTITION resume: continue from last completed checkpoint boundary.
        # Reject only when NO completed checkpoint exists (fail-closed).
        if requested is ExecutionStrategy.PARTITION and checkpoints:
            completed = {pid: cp for pid, cp in checkpoints.items() if cp.completed}
            if not completed:
                return StrategyResolution(
                    strategy=None,
                    status=StrategyResolutionStatus.REJECTED_NO_CHECKPOINT,
                    reason="No completed checkpoint boundary; refusing to resume.",
                    policy_version=policy.version,
                )
