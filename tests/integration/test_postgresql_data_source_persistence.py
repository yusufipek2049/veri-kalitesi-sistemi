...
from veri_kalitesi.data_sources.errors import ConflictError, ValidationError
...
class _FailingTransactionalAudit:
    @staticmethod
    def stage(prepared: PreparedAuditEvent, *, session: object) -> None:
        del prepared, session
        raise RuntimeError("synthetic PostgreSQL outbox failure")


def test_data_source_write_rolls_back_when_transactional_audit_stage_fails(
    repository: PostgreSQLDataSourceRepository,
    audit_outbox: PostgreSQLTransactionalAudit,
    db_settings: DatabaseSettings,
    sample_data_source: DataSource,
    ...
    with pytest.raises(RuntimeError, match="outbox failure"):
        repository.add_data_source(
            ...
            audit_outbox=_FailingTransactionalAudit(),  # type: ignore[arg-type]
        )

    with repository.session_factory() as session:
        stored_count = session.scalar(
            text(
                f'''SELECT count(*) FROM "{db_settings.schema}".data_sources
...
def test_pending_activation_unique_index_maps_only_duplicate_to_typed_conflict(
    repository: PostgreSQLDataSourceRepository,
    audit_outbox: PostgreSQLTransactionalAudit,
    sample_data_source: DataSource,
) -> None:
    ...
    first = DataSourceActivationRequest(
        data_source_id=sample_data_source.data_source_id,
        data_source_revision=1,
        maker_actor_id="maker-a",
        policy_version="S1_TEST_POLICY",
    )
    repository.add_activation_request(
        first,
        audit_event=_prepare_event(audit_outbox, "DATA_SOURCE_ACTIVATION_REQUESTED"),
        audit_outbox=audit_outbox,
    )
    duplicate = DataSourceActivationRequest(
        data_source_id=sample_data_source.data_source_id,
        data_source_revision=1,
        maker_actor_id="maker-b",
        policy_version="S1_TEST_POLICY",
    )
    ...
    assert error.value.code == "DATA_SOURCE_PENDING_ACTIVATION_EXISTS"


def test_concurrent_activation_decisions_commit_exactly_once(
    repository: PostgreSQLDataSourceRepository,
    audit_outbox: PostgreSQLTransactionalAudit,
    sample_data_source: DataSource,
) -> None:
    ...
    repository.update_connection_test(
        ...
        audit_event=_prepare_event(audit_outbox, "DATA_SOURCE_CONNECTION_TESTED"),
        audit_outbox=audit_outbox,
    )
    pending = repository.add_activation_request(
        DataSourceActivationRequest(
            data_source_id=sample_data_source.data_source_id,
            data_source_revision=1,
            maker_actor_id="maker-a",
    ...
    def decide(checker: str) -> str:
        decision = DataSourceActivationRequest(
            activation_request_id=pending.activation_request_id,
            data_source_id=pending.data_source_id,
            data_source_revision=pending.data_source_revision,
            maker_actor_id=pending.maker_actor_id,
            checker_actor_id=checker,
            policy_version=pending.policy_version,
            status=DataSourceActivationStatus.APPROVED,
            decision_reason_code="S1_CONCURRENT_APPROVAL",
            requested_at=pending.requested_at,
            decided_at=datetime.now(timezone.utc),
        )
        try:
            repository.decide_activation_request(
                decision,
                activate_source=True,
                audit_event=_prepare_event(audit_outbox, "DATA_SOURCE_ACTIVATION_DECIDED"),
                audit_outbox=audit_outbox,
            )
    ...
    assert sorted(outcomes) == ["COMMITTED", "DATA_SOURCE_DECISION_CONFLICT"]
    assert repository.get_data_source(sample_data_source.data_source_id).status is (
