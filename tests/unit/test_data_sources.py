...
from veri_kalitesi.data_protection.errors import InventoryCoverageTechnicalError
from veri_kalitesi.data_protection.inventory import InventoryCoverageStatus
from veri_kalitesi.data_protection.policy import ClassificationCode
from veri_kalitesi.data_sources.connectors import (
    CSVConnector,
    ConnectorRegistry,
)
...
from veri_kalitesi.data_sources.profiling import InMemoryProfilePolicyResolver
...
class FakePostgreSQLDriver:
    def __init__(
        ...
        metadata_outcomes: list[
            Exception | tuple[MetadataDatasetCandidate, ...] | MetadataDiscoveryOutcome
        ]
        | None = None,
        ...
        *,
        ...
        if isinstance(outcome, tuple):
            return MetadataDiscoveryOutcome(
                candidates=outcome,
                completed_scope={},
                scanned_object_count=len(outcome),
                is_complete=True,
            )
        return outcome
...
def test_fr_077_bfr_aud_004_data_source_creation_is_durably_buffered_on_outage() -> None:
    ...
    assert error.value.code == "DATA_SOURCE_AUDIT_UNAVAILABLE"
...
def test_fr_077_bfr_aud_004_connection_test_is_durably_buffered_on_outage(
    ...
    result = repository.latest_connection_test(data_source.data_source_id)
...
def test_fr_008_missing_secret_fails_closed_without_recording_authentication_failure() -> None:
    driver = FakePostgreSQLDriver()
    repository = SQLiteDataSourceRepository()
    service = _data_source_service(
        repository,
        ConnectorRegistry([PostgreSQLConnector(driver)]),
        InMemorySecretResolver({}),
    )
    data_source = service.create_data_source(
        actor_id="user-1",
        name="Missing Secret PostgreSQL",
        source_type="POSTGRESQL",
        connection_config=postgresql_config(),
        secret_reference="secret://datasources/missing",
        owner_user_id="owner-1",
    )
    ...
    assert error.value.code == "DATA_SOURCE_SECRET_UNAVAILABLE"
    assert (
        repository.get_data_source(data_source.data_source_id).status
        is DataSourceStatus.TEST_PENDING
    )
...
def test_fr_012_successful_connection_revision_promotes_and_invalidates_old_approval(
    ...
    assert error.value.code == "DATA_SOURCE_DECISION_CONFLICT"
...
def test_fr_010_activation_decision_exact_replay_is_idempotent() -> None:
    service, source = _tested_activation_source_without_file()
    request = service.request_activation(
        actor_context=_activation_context(source.data_source_id, "maker-a", {"SOURCE_MAKER"}),
        data_source_id=source.data_source_id,
    )
    checker = _activation_context(source.data_source_id, "checker-a", {"SOURCE_CHECKER"})
    first = service.decide_activation(
        actor_context=checker,
        activation_request_id=request.activation_request_id,
        decision="APPROVE",
        reason_code="SOURCE.READ_ONLY.TESTED",
    )
    audit_count = len(_audit_events(service))
    replay = service.decide_activation(
        actor_context=checker,
        activation_request_id=request.activation_request_id,
        decision="APPROVE",
        reason_code="SOURCE.READ_ONLY.TESTED",
    ...
    assert replay == first
    assert len(_audit_events(service)) == audit_count
    with pytest.raises(ConflictError) as error:
        service.decide_activation(
            actor_context=checker,
            activation_request_id=request.activation_request_id,
