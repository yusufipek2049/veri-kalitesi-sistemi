...
def test_data_source_tables_unique_constraints() -> None:
    """data_sources.name, datasets (source_id, namespace, name),
    data_fields (dataset_id, name) unique."""
    tables = data_source_tables()
...
def test_data_source_tables_check_constraints() -> None:
    """data_sources, data_fields, datasets, data_profiles
    ve connection_revisions CheckConstraint icerir."""
    tables = data_source_tables()

    source_cc = [c for c in tables.sources.constraints if isinstance(c, CheckConstraint)]
    source_type_constraint = next(c for c in source_cc if "source_type" in str(c.sqltext))
    source_status_constraint = next(c for c in source_cc if "status" in str(c.sqltext))
    assert "'EXCEL'" in str(source_type_constraint.sqltext)
    assert "'REST'" in str(source_type_constraint.sqltext)
    assert "'REST_API'" not in str(source_type_constraint.sqltext)
    assert "'OTHER'" not in str(source_type_constraint.sqltext)
    assert "'TEST_PENDING'" in str(source_status_constraint.sqltext)
...
def test_data_source_tables_json_columns() -> None:
    """data_sources.connection_config, connection_test_results.source_info,
    data_profiles.metrics, connection_revisions.connection_config JSON türündedir."""
    tables = data_source_tables()
...
def test_data_source_tables_boolean_columns() -> None:
    """data_fields.is_nullable, data_fields.is_sensitive,
    processing_inventory.cross_border_transfer Boolean türündedir."""
    tables = data_source_tables()
