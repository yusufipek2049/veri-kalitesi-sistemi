...
class _Repository:
    ...
    def register_worker(self, registration, *, now=None, audit_event=None, audit_outbox=None):
        return registration

    def heartbeat_worker(self, worker_id, *, now, audit_event=None, audit_outbox=None):
        return WorkerRegistration(
            worker_id=worker_id,
            hostname="test-worker",
            capacity=2,
            supported_job_types=("EXECUTION",),
            state=WorkerState.RUNNING,
            last_seen_at=now,
            version=1,
        )

    def begin_drain(self, worker_id, *, now, audit_event=None, audit_outbox=None):
        return True

    def stop_worker(self, worker_id, *, now, audit_event=None, audit_outbox=None):
        return True
    ...
    def block_job(
        ...
        *,
        ...
        self.job = replace(self.job, status=JobStatus.BLOCKED)
        return self.job
