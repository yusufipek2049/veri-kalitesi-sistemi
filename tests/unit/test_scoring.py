...
# ── DS-06: calculate_full_score_set batch testleri ──────────────────


def test_ds06_batch_score_set_returns_all_levels() -> None:
    """FR-04.06: calculate_full_score_set rule→dataset→dimension→source→enterprise döner."""
    service, score_repo, exec_repo, versions = _aggregate_service(weights=(1.0, 1.0))
    execution = _aggregate_execution(versions)
    exec_repo.create_or_get(execution)
    results = tuple(
        _version_result(execution, version, checked=100, passed=95, failed=5)
        for version in versions
    )
    completed = exec_repo.complete_success(
        execution.execution_id,
        results,
        datetime(2026, 7, 16, 10, 5, tzinfo=timezone.utc),
    )
    all_scores = service.calculate_full_score_set(completed.execution_id)
    scope_types = {s.scope_type for s in all_scores}
    assert ScoreScopeType.RULE in scope_types
    assert ScoreScopeType.DATASET in scope_types
    assert ScoreScopeType.ENTERPRISE not in scope_types  # no source catalog
    assert len(all_scores) >= 2  # en az 2 rule score


def test_ds06_batch_score_set_is_deterministic() -> None:
    """AC-06: Aynı input ile iki çağrı aynı skor setini üretir."""
    service, _, exec_repo, versions = _aggregate_service(weights=(1.0, 1.0))
    execution = _aggregate_execution(versions)
    exec_repo.create_or_get(execution)
    results = tuple(
        _version_result(execution, version, checked=100, passed=90, failed=10)
        for version in versions
    )
    completed = exec_repo.complete_success(
        execution.execution_id,
    ...
    first = service.calculate_full_score_set(completed.execution_id)
    second = service.calculate_full_score_set(completed.execution_id)
...
def test_ds06_batch_score_set_persists_aggregate_scores() -> None:
    """FR-04.06: Batch hesaplama aggregate skorları repository'ye yazar."""
    service, score_repo, exec_repo, versions = _aggregate_service(weights=(1.0, 1.0))
    execution = _aggregate_execution(versions)
    exec_repo.create_or_get(execution)
    results = tuple(
    ...
    # Aggregate skorlar (dataset, dimension) repository'ye yazıldı
    persisted = score_repo.list_for_execution(completed.execution_id)
    assert len(persisted) > 0
    persisted_scope_types = {s.scope_type for s in persisted}
    assert ScoreScopeType.DATASET in persisted_scope_types


def test_ds06_batch_score_set_with_source_catalog_produces_enterprise() -> None:
    """FR-04.06: Source catalog verilirse enterprise score de hesaplanır."""
    service, _, exec_repo, versions, _ = _source_service()
    execution = _source_execution(versions)
    exec_repo.create_or_get(execution)
    results = tuple(
        _version_result(execution, version, checked=100, passed=90, failed=10)
    ...
    all_scores = service.calculate_full_score_set(completed.execution_id)
    scope_types = {s.scope_type for s in all_scores}
    assert ScoreScopeType.ENTERPRISE in scope_types
    assert ScoreScopeType.SOURCE in scope_types


def test_ds06_reproduce_rule_score_is_deterministic() -> None:
    """AC-06: reproduce_rule_score aynı input ile aynı sonucu verir."""
    service, _, exec_repo, version = _service()
    execution = _execution(version)
    exec_repo.create_or_get(execution)
    result = _result(execution, checked=100, passed=95, failed=5)
    completed = exec_repo.complete_success(
        execution.execution_id,
        (result,),
        datetime(2026, 7, 16, 9, 5, tzinfo=timezone.utc),
    )
    reproduced = service.reproduce_rule_score(
        execution=completed,
        version=version,
        result=result,
