...
def test_synthetic_keycloak_realm_projects_versioned_group_and_mfa_claims_without_secret() -> None:
    realm_path = REPOSITORY_ROOT / "infra" / "enterprise-lab" / "config" / "keycloak-realm.json"
...
def test_composition_root_wires_evidence_age_threshold_from_verified_configuration(
    tmp_path: Path,
) -> None:
    """Kapinin kanit omru esigi dogrulanmis yapilandirmadan gelir (MAINT-04)."""
    token_path = tmp_path / "secret-manager-token"
    token_path.write_text("synthetic-token", encoding="utf-8")
    ...
    # Composition root, dogrulanmis evidence.max_evidence_age_seconds degerini
    # gate'e aktarir; environment.json'daki 3600 degeri ile eslesmelidir.
    gate = adapters.identity._gate  # type: ignore[attr-defined]
    assert gate._max_evidence_age_seconds == 3600  # type: ignore[attr-defined]
