...
def test_ds02_ac_postgresql_reader_failure_returns_safe_technical_error() -> None:
    response = TestClient(_app(FailingPostgreSQLExecutionReader(), frozenset({"source-a"}))).get(
        "/api/v1/executions"
    )

    assert response.status_code == 503
    assert response.json()["title"] == "Executions temporarily unavailable"
    assert "postgresql contains sensitive detail" not in response.text
...
class FailingPostgreSQLExecutionReader:
    def list_executions_for_sources(
        self,
        allowed_source_ids: frozenset[str],
        *,
        ...
        raise SQLAlchemyError("postgresql contains sensitive detail")
...
def _app(
    reader: FakeExecutionReader | FailingExecutionReader | FailingPostgreSQLExecutionReader,
