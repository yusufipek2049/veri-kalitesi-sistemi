...
# Prototip laboratuvari kanit omeri (DQ-CAP-012); LabAdapterGate dogrudan
# kurulan testlerde esik degeri acik verilir.
...
class TestExecutionStrategyEngine:
    ...
    def test_partition_with_mixed_checkpoints_resumes_from_completed(self) -> None:
        """DQ-CAP-013: resume from last completed boundary, not blocked by incomplete."""
        checkpoints = {
    ...
    def test_partition_with_only_incomplete_checkpoints_rejects(self) -> None:
        """DQ-CAP-013: no completed boundary → fail-closed reject."""
        checkpoints = {
            "p1": CheckpointState(partition_id="p1", completed=False),
            "p2": CheckpointState(partition_id="p2", completed=False),
        }
        result = self.engine.resolve(
            ExecutionStrategy.PARTITION,
    ...
    def test_partition_with_no_checkpoints_resolves(self) -> None:
        """No checkpoints provided means no resume constraint; resolve from policy."""
        result = self.engine.resolve(
            ExecutionStrategy.PARTITION,
            policy=_strategy_policy(),
            checkpoints=None,
    ...
    def test_partition_with_empty_checkpoints_resolves(self) -> None:
        """Empty checkpoint mapping means no resume constraint; resolve from policy."""
        result = self.engine.resolve(
            ExecutionStrategy.PARTITION,
            policy=_strategy_policy(),
            checkpoints={},
