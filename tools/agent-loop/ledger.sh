...
evidence_write() {
  ...
  dir="$EVIDENCE_DIR/$id"
  mkdir -p "$dir"
  # Onarım turu adın parçasıdır: aksi halde tur 1'in kanıtı tur 0'ınkini ezer
  # ve "hangi turda neyin geçtiği" geri döndürülemez. review_record aynı deseni
  # zaten kullanıyor.
  file="$dir/i$(state_field iteration)-r$(state_field repair_round)-$suite.json"
