...
export function IssuesPage({
  ...
  const [createScopeType, setCreateScopeType] = useState<"DATASET" | "SOURCE">("DATASET");
  ...
  const submitCreateIssue = async () => {
    if (!onCreateIssue || createPending) return;
    if (!createTitle.trim() || !createScopeId.trim()) return;
    ...
    <AppShell currentPage="Sorunlar">
      <Box sx={(theme) => ({ display: "grid", gap: 5, margin: "0 auto", maxWidth: theme.appLayout.contentMaxWidth, p: { xs: 3, md: 4, lg: 6 }, width: "100%" })}>
          ...
          {state !== "unauthorized" ? (
            <Box sx={{ display: "flex", gap: 2 }}>
              {canCreateIssue && onCreateIssue ? (
                <Button onClick={openCreateDialog} startIcon={<Plus aria-hidden="true" size={16} />} variant="outlined">Yeni Sorun</Button>
              ) : null}
        ...
        >
          ...
          <DialogContent sx={{ display: "grid", gap: 4, pt: 2 }}>
            ...
            <FormControl>
              ...
              <Select
                ...
                onChange={(event) => setCreateScopeType(event.target.value as "DATASET" | "SOURCE")}
            ...
            <FormControl>
              ...
              <Select
                ...
                onChange={(event) => setCreatePriority(event.target.value as IssuePriority)}
          ...
          <DialogActions>
            <Button onClick={closeCreateDialog}>Vazgeç</Button>
            <Button
              disabled={
                !createTitle.trim()
                || !createScopeId.trim()
              ...
              onClick={() => void submitCreateIssue()}
              variant="contained"
            >
