...
export interface DashboardSummaryApiResponse {
  ...
  applied_filters: AppliedDashboardFilters | null;
}

export type DashboardFilterScopeType = "SOURCE" | "ENTERPRISE";
export type DashboardFilterScoreStatus =
  | "CALCULATED"
  | "NOT_CALCULATED"
  | "NO_DATA"
  | "PARTIAL"
  ...
  | "CONFIG_ERROR";
export type DashboardFilterLevel = "GOOD" | "ACCEPTABLE" | "RISKY" | "CRITICAL";

export interface DashboardFilters {
  scope_type?: DashboardFilterScopeType;
  scope_id?: string;
  score_status?: DashboardFilterScoreStatus;
  level?: DashboardFilterLevel;
}

export const DASHBOARD_FILTER_SCOPE_TYPES: DashboardFilterScopeType[] = ["SOURCE", "ENTERPRISE"];
export const DASHBOARD_FILTER_SCORE_STATUSES: DashboardFilterScoreStatus[] = [
  "CALCULATED",
  "NOT_CALCULATED",
  "NO_DATA",
  "PARTIAL",
...
export const DASHBOARD_FILTER_LEVELS: DashboardFilterLevel[] = ["GOOD", "ACCEPTABLE", "RISKY", "CRITICAL"];

export function isValidScopeType(value: string): value is DashboardFilterScopeType {
  return DASHBOARD_FILTER_SCOPE_TYPES.includes(value as DashboardFilterScopeType);
}

export function isValidScoreStatus(value: string): value is DashboardFilterScoreStatus {
  return DASHBOARD_FILTER_SCORE_STATUSES.includes(value as DashboardFilterScoreStatus);
}

export function isValidLevel(value: string): value is DashboardFilterLevel {
  return DASHBOARD_FILTER_LEVELS.includes(value as DashboardFilterLevel);
}
...
export function filtersToSearchParams(filters: DashboardFilters): URLSearchParams {
  ...
  if (filters.score_status) params.set("score_status", filters.score_status);
...
export function filtersFromSearchParams(params: URLSearchParams): DashboardFilters {
  ...
  if (scopeType && isValidScopeType(scopeType)) filters.scope_type = scopeType;
  const scopeId = params.get("scope_id");
  if (scopeId) filters.scope_id = scopeId;
  const scoreStatus = params.get("score_status");
  if (scoreStatus && isValidScoreStatus(scoreStatus)) filters.score_status = scoreStatus;
  const level = params.get("level");
  if (level && isValidLevel(level)) filters.level = level;
  return filters;
}

export function hasInvalidFilterParams(params: URLSearchParams): boolean {
  const scopeType = params.get("scope_type");
  if (scopeType !== null && !isValidScopeType(scopeType)) return true;
  const scoreStatus = params.get("score_status");
  if (scoreStatus !== null && !isValidScoreStatus(scoreStatus)) return true;
  const level = params.get("level");
  if (level !== null && !isValidLevel(level)) return true;
  return false;
}

export function isFiltersEmpty(filters: DashboardFilters): boolean {
  return !filters.scope_type && !filters.scope_id && !filters.score_status && !filters.level;
}
...
export const trendObservations: TrendObservation[] = [
  { date: "2026-06-23", displayDate: "23 Haz", rawScore: 72.1, finalScore: 72.1, qualification: "Qualified", usageDecision: "Allowed", coverageRate: 98, technicalStatus: "Başarılı", official: true, trend: { moving_average: null, consecutive_deterioration_count: null, sudden_deterioration: null, time_below_threshold_periods: null, improvement_persistence: null, version_boundary: true, policy_version: "1.0.0" }, versionBoundary: true },
  { date: "2026-06-27", displayDate: "27 Haz", rawScore: 76.8, finalScore: 76.8, qualification: "Qualified", usageDecision: "Allowed", coverageRate: 97, technicalStatus: "Başarılı", official: true, trend: { moving_average: 74.5, consecutive_deterioration_count: 0, sudden_deterioration: false, time_below_threshold_periods: 0, improvement_persistence: 1, version_boundary: false, policy_version: "1.0.0" }, versionBoundary: false },
  { date: "2026-07-01", displayDate: "1 Tem", rawScore: 78.2, finalScore: 78.2, qualification: "Qualified", usageDecision: "Allowed", coverageRate: 97, technicalStatus: "Başarılı", official: true, trend: { moving_average: 75.7, consecutive_deterioration_count: 0, sudden_deterioration: false, time_below_threshold_periods: 0, improvement_persistence: 2, version_boundary: false, policy_version: "1.0.0" }, versionBoundary: false },
  { date: "2026-07-05", displayDate: "5 Tem", rawScore: null, finalScore: null, qualification: "TechnicalFailure", usageDecision: "Undetermined", coverageRate: null, technicalStatus: "Teknik Hata", official: false, trend: null, versionBoundary: false },
  { date: "2026-07-09", displayDate: "9 Tem", rawScore: 82.4, finalScore: 82.4, qualification: "Qualified", usageDecision: "Allowed", coverageRate: 96, technicalStatus: "Başarılı", official: true, trend: { moving_average: 79.1, consecutive_deterioration_count: 0, sudden_deterioration: false, time_below_threshold_periods: 0, improvement_persistence: 3, version_boundary: false, policy_version: "1.0.0" }, versionBoundary: false },
  { date: "2026-07-13", displayDate: "13 Tem", rawScore: 84.6, finalScore: 84.6, qualification: "ProvisionallyQualified", usageDecision: "ConditionallyAllowed", coverageRate: 92, technicalStatus: "Başarılı", official: false, trend: null, versionBoundary: false },
  { date: "2026-07-17", displayDate: "17 Tem", rawScore: 86.2, finalScore: 86.2, qualification: "Qualified", usageDecision: "Allowed", coverageRate: 96, technicalStatus: "Başarılı", official: true, trend: { moving_average: 82.3, consecutive_deterioration_count: 0, sudden_deterioration: false, time_below_threshold_periods: 0, improvement_persistence: 4, version_boundary: false, policy_version: "1.0.0" }, versionBoundary: false },
  { date: "2026-07-21", displayDate: "21 Tem", rawScore: 89.1, finalScore: 87.4, qualification: "LimitedCoverage", usageDecision: "ConditionallyAllowed", coverageRate: 94, technicalStatus: "Başarılı", official: true, trend: { moving_average: 84.2, consecutive_deterioration_count: 0, sudden_deterioration: false, time_below_threshold_periods: 0, improvement_persistence: 5, version_boundary: false, policy_version: "1.0.0" }, versionBoundary: false },
];
...
export function dashboardViewModelFromApi(
  ...
  const observations = response.periods.map((period) => {
    ...
    return {
      ...
      trend: observation?.trend
        ? {
            moving_average: observation.trend.moving_average,
            consecutive_deterioration_count: observation.trend.consecutive_deterioration_count,
            sudden_deterioration: observation.trend.sudden_deterioration,
            time_below_threshold_periods: observation.trend.time_below_threshold_periods,
            improvement_persistence: observation.trend.improvement_persistence,
            version_boundary: observation.trend.version_boundary,
            policy_version: observation.trend.policy_version,
          }
        : null,
      versionBoundary: observation?.trend?.version_boundary === true,
    };
  });
  ...
  return {
    ...
    policyVersion: latest?.trend?.policy_version ?? null,
  };
}
