...
describe("FR-057 dashboard filtre modelleri", () => {
  it("geçerli scope_type değerlerini kabul eder", () => {
    expect(isValidScopeType("SOURCE")).toBe(true);
    expect(isValidScopeType("ENTERPRISE")).toBe(true);
    expect(isValidScopeType("INVALID")).toBe(false);
  });

  it("geçerli score_status değerlerini kabul eder", () => {
    expect(isValidScoreStatus("CALCULATED")).toBe(true);
    expect(isValidScoreStatus("NOT_CALCULATED_TECHNICAL_ERROR")).toBe(true);
    expect(isValidScoreStatus("BOGUS")).toBe(false);
  });

  it("geçerli level değerlerini kabul eder", () => {
    expect(isValidLevel("GOOD")).toBe(true);
    expect(isValidLevel("CRITICAL")).toBe(true);
    expect(isValidLevel("INVALID")).toBe(false);
  });

  it("filtreleri URL parametrelerine dönüştürür", () => {
    const params = filtersToSearchParams({ scope_type: "SOURCE", level: "GOOD" });
    expect(params.get("scope_type")).toBe("SOURCE");
    expect(params.get("level")).toBe("GOOD");
    expect(params.has("scope_id")).toBe(false);
    expect(params.has("score_status")).toBe(false);
  });

  it("URL parametrelerinden filtreleri okur ve yalnız geçerli değerleri kabul eder", () => {
    const params = new URLSearchParams("scope_type=SOURCE&score_status=BOGUS&level=GOOD");
    const filters = filtersFromSearchParams(params);
    expect(filters.scope_type).toBe("SOURCE");
    expect(filters.score_status).toBeUndefined();
    expect(filters.level).toBe("GOOD");
  });

  it("geçersiz URL parametresi tespit edilir", () => {
    expect(hasInvalidFilterParams(new URLSearchParams("scope_type=INVALID"))).toBe(true);
    expect(hasInvalidFilterParams(new URLSearchParams("score_status=BOGUS"))).toBe(true);
    expect(hasInvalidFilterParams(new URLSearchParams("level=NOPE"))).toBe(true);
    expect(hasInvalidFilterParams(new URLSearchParams("scope_type=SOURCE"))).toBe(false);
    expect(hasInvalidFilterParams(new URLSearchParams(""))).toBe(false);
  });

  it("boş filtre nesnesi tespit edilir", () => {
    expect(isFiltersEmpty({})).toBe(true);
    expect(isFiltersEmpty({ scope_type: "SOURCE" })).toBe(false);
  });
});
