...
class TransactionalAuditSink(Protocol):
    """Minimal audit outbox contract for incident response persistence.

    Structural protocol satisfied by ``SQLiteTransactionalAudit``.  Keeps
    the incident response repository decoupled from the concrete audit
...
class SQLiteIncidentResponseRepository:
    ...
    def create_incident(
        ...
        audit_outbox: TransactionalAuditSink,
    ) -> None:
    ...
    def _require_shared_audit_transaction(self, audit_outbox: TransactionalAuditSink) -> None:
