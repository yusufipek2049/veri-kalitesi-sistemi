...
`ClassificationPolicy` ve `MaskingPolicy` domain servislerinden bağımsız, sürümlü politika adaptörleridir.

## Teknik Baseline

- `veri_kalitesi.data_protection` paketi `CLASSIFICATION_POLICY_V1` sözlüğünü ve `PROFILE_MASKING_POLICY_V1` profil minimizasyonunu sağlar.
...
- Bu kodların banka sınıflandırma sözlüğüyle eşlemesi henüz onaylanmamıştır; teknik doğrulama banka politika onayı değildir.
...
- Teknik sınıf kodlarının banka onaylı müşteri sırrı/banka sırrı etiketleriyle eşlemesi.
