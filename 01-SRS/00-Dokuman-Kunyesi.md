---
type: srs-metadata
project: Veri Kalitesi İzleme ve Skorlama Sistemi
source_document: Veri_Kalitesi_Izleme_ve_Skorlama_Sistemi_SRS_v0.1.md
source_version: "0.1"
source_section: "Kapak, değişiklik geçmişi ve içindekiler"
created_at: 2026-07-16
tags:
  - srs
  - kunye
---

# SRS Doküman Künyesi

## Veri Kalitesi İzleme ve Skorlama Sistemi



| Alan | Değer |
| --- | --- |
| Doküman adı | Veri Kalitesi İzleme ve Skorlama Sistemi — Yazılım Gereksinimleri Belirtimi |
| Proje adı | Veri Kalitesi İzleme ve Skorlama Sistemi |
| Doküman sürümü | 0.1 Taslak |
| Hazırlanma tarihi | 16 Temmuz 2026 |
| Hazırlayan | Yusuf İpek |
| Kurum | Kurum |
| Doküman durumu | Taslak — inceleme ve onaya açık |
| Onaylayan roller | Proje Sponsoru; Veri Yönetişimi Yöneticisi; Bilgi Güvenliği Yöneticisi; BT Mimari Kurulu; İş Birimi Temsilcisi |

### Değişiklik Geçmişi

| Sürüm | Tarih | Hazırlayan | Değişiklik | Durum |
| --- | --- | --- | --- | --- |
| 0.1 | 16.07.2026 | Yusuf İpek | İlk taslak oluşturuldu; kurum içi veri merkezi, LDAP, ServiceNow ve beş yıllık saklama varsayımları işlendi. | Taslak |

## İçindekiler


1. Amaç  
2. Genel Sistem Açıklaması  
3. İş Gereksinimleri  
4. Fonksiyonel Gereksinimler  
5. Kullanım Senaryoları  
6. İş Kuralları  
7. Veri Gereksinimleri  
8. Harici Arayüz Gereksinimleri  
9. Fonksiyonel Olmayan Gereksinimler  
10. Kabul Kriterleri  
11. Gereksinim İzlenebilirlik Matrisi  
12. Önceliklendirme ve MVP  
13. Riskler  
14. Sistem Evrimi  
15. Açık Konular ve Varsayımlar  
16. Doküman Kalite Kontrolü

> **Doküman notu:** Bu sürüm, yanıtlanan netleştirme sorularına göre hazırlanmıştır. Ortama veya kullanım bağlamına göre değişen kapasite, güvenlik ve performans değerleri aktif, sürümlü ve onaylı politika kayıtlarından çözülür; politika kaydı yoksa sistem fail-closed davranır. Saklama ve RPO/RTO teknik değerleri karar tablolarında tanımlıdır.
