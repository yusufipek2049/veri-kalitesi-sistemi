...
| Ana dashboard | Genel kalite ve operasyon durumunu sunmak | Tüm yetkili kullanıcılar | Ham kalite skoru, kapsam, güven, kritiklik/risk, teknik sağlık, boyut/kural kırılımı, trend, kritik sorun ve son işler | Filtre, sıralama, drill-down, görünüm değiştirme | Tarih, kaynak, birim, sahip, boyut, ölçüm durumu, kalite seviyesi, risk/kritiklik, model sürümü | Tarih aralığı, karşılaştırılabilir sürüm ve kapsam yetkisi | Veri yok; düşük güven; eski skor; sorgu zaman aşımı; yetkisiz kapsam |
...
| Skor detay ekranı | Skorun formül ve alt bileşenlerini açıklamak | Data Owner, Data Steward, Denetçi | Ham kalite skoru, ölçüm durumu, boyut/kural kırılımı, kritik kural kararı, kapsam, güven, kritiklik/risk, teknik sağlık, istisna/override, tüm politika/model sürümleri, son ve son başarılı hesaplama zamanı | Dönem karşılaştır, kural/issue/remediation detayına git | Tarih, boyut, durum, model sürümü | Provizyonel skor resmî trend/SLA'ya katılamaz; hesaplanmamış skor 0 gösterilemez; ham skor override ile değiştirilemez | `NoData`; `NotApplicable`; `NotMeasured`; `TechnicalError`; düşük güven; config/politika hatası |
...
- Kalite skoru, kapsam, güven, kritiklik/risk ve teknik sağlık tek yüzdeye
  birleştirilmez. Skor çalışan performans KPI'sı olarak sunulmaz.
