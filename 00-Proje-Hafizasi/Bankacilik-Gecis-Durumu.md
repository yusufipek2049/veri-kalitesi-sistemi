...
- `DQ-SCR-001`–`DQ-SCR-033` bağlayıcı skorlama kararları gereksinim, mimari, veri modeli, API, arayüz ve test sözleşmelerine işlenmiştir. Bu çalışma hedef modeli tanımlar; standart ölçüm durumları, kapsam/güven, ayrı kritiklik/risk ve override yapıları henüz runtime'a taşınmamıştır. İlişkili `OPEN-BNK` kayıtları banka onayı olmadan kapatılmamıştır.
...
| 7 | Kabul edilen skorlama hedef modeli runtime'a taşınmadı | Mevcut prototip dataset kritikliğini `SOURCE` kalite agregasyonunda kullanır; standart durumlar, kapsam/güven, ayrı risk ve override modeli yoktur | İterasyon 33A ve sonraki skorlama dilimleri |
