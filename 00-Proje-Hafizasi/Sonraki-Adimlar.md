...
Mevcut 16 iterasyonun, Iterasyon 17A–17E audit, Iterasyon 18A–18C veri koruma, Iterasyon 19A–19H maker-checker, Iterasyon 20A–20C LDAP/oturum güvenliği ve Iterasyon 25A saklama dry-run dikeylerinin çekirdeği korunur. Banka kararları tamamlanmadan yeni HTTP yüzeyi açılmaz.
...
8. İterasyon 25 — 25A saklama politikası dry-run `TechnicallyVerified`; legal hold yaşam döngüsü, imha ve arşiv geri çağırma açık.
...
| 25 | Saklama/imha/legal hold | `RULE-014`, `BFR-LCM-001`–`BFR-LCM-004` | Kayıt türü bazlı politika; imha ve geri çağırma kanıtı | 25A dry-run `TechnicallyVerified`; kalıcı hold, imha/audit ve geri çağırma açık; banka onayı bekleniyor |
...
Sonraki teknik aday **İterasyon 25B — Kalıcı legal hold yaşam döngüsü ve audit**
dilimidir. Kapsam güvenilir aktörle append-only hold kaydı, farklı yetkili aktörle
serbest bırakma, politika sürümü bağı ve atomik veri-minimum audit ile sınırlı
tutulmalıdır. Fiziksel imha, anonimleştirme ve arşiv geri çağırma banka onayı ile
ayrı artımlar olarak kalmalıdır. 29D `OPEN-BNK-012`, 21B/frontend `OPEN-BNK-020`,
