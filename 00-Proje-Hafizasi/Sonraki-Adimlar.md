...
Mevcut 16 iterasyonun, Iterasyon 17A–17E audit, Iterasyon 18A–18C veri koruma, Iterasyon 19A–19H maker-checker, Iterasyon 20A–20C LDAP/oturum güvenliği ve Iterasyon 25A–25B saklama/legal hold dikeylerinin çekirdeği korunur. Banka kararları tamamlanmadan yeni HTTP yüzeyi açılmaz.
...
8. İterasyon 25 — 25A saklama politikası dry-run ve 25B kalıcı legal hold yaşam döngüsü `TechnicallyVerified`; imha ve arşiv geri çağırma açık.
...
| 25 | Saklama/imha/legal hold | `RULE-014`, `BFR-LCM-001`–`BFR-LCM-004` | Kayıt türü bazlı politika; imha ve geri çağırma kanıtı | 25A dry-run ve 25B legal hold `TechnicallyVerified`; imha/audit ve geri çağırma açık; banka onayı bekleniyor |
...
Sonraki teknik aday **İterasyon 25C — İdempotent imha işi ve veri-minimum kanıt
zarfı sözleşmesi** dilimidir. Kapsam onaylı politika/hold önkoşulu, değişmez aday
snapshot'ı, idempotency, sonuç sayaçları ve atomik audit ile sınırlı tutulmalı;
gerçek silme/anonimleştirme adaptörleri banka onayı olmadan etkinleştirilmemelidir.
Arşiv geri çağırma ayrı artım kalmalıdır. 29D `OPEN-BNK-012`, 21B/frontend `OPEN-BNK-020`,
hassas dışa aktarma `OPEN-BNK-014` ve gerçek SIEM/SOC `OPEN-BNK-010` kararlarını
beklemektedir.
