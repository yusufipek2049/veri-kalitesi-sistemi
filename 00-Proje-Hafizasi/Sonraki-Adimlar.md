...
Mevcut 16 iterasyonun, Iterasyon 17A–17E audit, Iterasyon 18A–18C veri koruma, Iterasyon 19A–19E maker-checker ve Iterasyon 20A–20C LDAP/oturum güvenliği dikeylerinin çekirdeği korunur. Banka kararları tamamlanmadan yeni HTTP yüzeyi açılmaz.

Bakım İterasyonu 29C.1 ile tam mypy baseline'ı sıfır hataya indirilmiştir;
sonraki değişiklikler bu baseline'ı korumalıdır.

1. İterasyon 19E — Veri kaynağı aktivasyonu maker-checker; `TechnicallyVerified`.
2. İterasyon 19F — Kaynak aktivasyon isteği geri çekme ve 3/10 iş günlük süre aşımı; sıradaki teknik aday.
...
| 19 | Maker-checker | `RULE-001`, `RULE-005`, `RULE-007`, `BFR-SOD-001`–`BFR-SOD-004` | Hazırlayan kendi kritik değişikliğini aktive edemez | 19A kritik kural, 19B scoring, 19C kural geri çekme, 19D kural süre aşımı ve 19E veri kaynağı aktivasyonu `TechnicallyVerified`; kaynak onay geri çekme/süre aşımı ve banka rol eşlemesi açık |
...
Sonraki teknik aday **İterasyon 19F — Veri kaynağı aktivasyon isteği geri çekme
ve süre aşımı** dilimidir. Kapsam, 19E'nin kaynak revizyonuna bağlı bekleyen
isteğini kayıtlı maker tarafından geri çekme ve 19D'nin sürümlü 3/10 iş günlük
takvim politikasını kaynak onaylarına uygulamakla sınırlı tutulmalıdır. Banka
onaylı maker/checker/worker rol kodları ve gerçek takvim kaynağı
`ComplianceReviewRequired` kalır. 29D, HTTP/frontend ve hassas dışa aktarma kendi
açık banka/altyapı kararları tamamlanmadan uygulanmamalıdır.
