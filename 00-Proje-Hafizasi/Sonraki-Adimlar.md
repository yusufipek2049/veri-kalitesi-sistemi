...
tamamlandı. Kalan davranışlar; CPU/IO ve hız sınırı gözlemi,
...
Sıradaki değerlendirilecek artım **İterasyon 31D — Kaynak hız sınırı politikasının
worker admission sınırına bağlanması**dır. Kabul edilen hız sınırı şeması ve
kalıcı sayaç davranışı ilgili gereksinimlerden doğrulanmalıdır. CPU/IO için
güvenilir kaynak ölçüm adaptörü ayrı dilimde kalmalıdır.
