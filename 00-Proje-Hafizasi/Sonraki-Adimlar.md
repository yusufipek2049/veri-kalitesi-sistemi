...
kota çözümleme ve çalışma penceresi dilimleri 31A–31B ile tamamlandı. Kalan
davranışlar; CPU/IO ve hız sınırı gözlemi, dinamik timeout/retry,
...
Sıradaki hazır artım **İterasyon 31C — Kaynak politikasındaki timeout ve retry
değerlerinin worker yürütmesine bağlanması**dır. Birden fazla kaynakta en dar
timeout/retry sınırının seçimi açıkça test edilmelidir. CPU/IO ile hız sınırı için
ölçüm adaptörü ayrı dilimde kalmalıdır.
