...
| FE-DS-006 | Ortak chart wrapper ve formatter | Must | FE-DS-001 | 30B trend ve 30D karşılaştırma/matris alt kapsamları `TechnicallyVerified` |
| FE-DS-007 | Ortak data table standardı | Must | FE-DS-001, API sayfalama sözleşmesi | Planlı / engelli |
| FE-DS-008 | Kurumsal dashboard referans ekranı | Must | 21B, FE-DS-003–007 | 30D referans içerik alt kapsamı `TechnicallyVerified`; üretim bağlantısı engelli |
...
| FE-DS-015 | Route, yetkisiz erişim ve bulunamadı durumlarını taşıyan istemci navigasyon kabuğu | Must | İlk alan ekranı | 35A `TechnicallyVerified` |
| FE-DASH-001 | Ölçüm yeterliliği, kritik kontrol ve teknik hata KPI sözleşmesi ile API bağlantısı | Must | Yeterlilik runtime'ı, dashboard API | 21C DTO ve 30D UI bağlantısı `TechnicallyVerified`; kritik kontrol runtime kaynağı açık |
...
5. **İterasyon 35A–35F — Alan ekranları:** Veri Kaynakları, Kurallar,
   Çalıştırmalar, Sorunlar, Raporlar ve Denetim `TechnicallyVerified`.
   Her dilim route, güvenli API,
   yetki, loading/empty/error durumları, Storybook ve Playwright kanıtıyla ayrı
   teslim edilecektir.

30C, 20E, 21C, 30D ve 35A–35F tamamlandı. Alan ekranlarının üretim bağlantısı gerçek IdP callback,
...
dışa aktarma uygulaması tamamlanmadan sonraki rapor yazma dilimine alınmaz;
35E güvenli rapor önizlemesiyle sınırlıdır.
...
2. **36A2a — PostgreSQL issue mutasyon ve audit outbox:**
...
5. **36C — Yazılabilir Kurallar:**
   ...
   - **36C1b — Kural düzenleme/onay frontend formları:** `TechnicallyVerified`.
...
8. **36F — Rapor İşlemleri ve Denetim Sınırı:** rapor üretim talebi ile güvenli
...
**İterasyon 20E — BFF oturum ve CSRF HTTP sınırı**, **İterasyon 21C —
...
okunur Çalıştırmalar ekranı**, **İterasyon 35D — salt okunur Sorunlar ekranı**
ve **İterasyon 35E — güvenli Rapor Önizleme ekranı** ile **İterasyon 35F — salt
okunur Denetim ekranı** tamamlandı. Kullanıcı öncelikli alan ekranları zinciri
tamamlanmıştır.
...
Kurallar, 35C kaynak scope filtreli veri-minimum Çalıştırmalar, 35D
kaynak/dataset scope filtreli veri-minimum Sorunlar ve 35E rol/source scope
filtreli toplulaştırılmış Rapor Önizleme API'sini ve salt okunur liste
ekranlarını, 35F ise rol kontrollü ve snapshot sayfalı Denetim ekranını
tamamladı. Üretim bağlantısı ayrıca gerçek IdP
