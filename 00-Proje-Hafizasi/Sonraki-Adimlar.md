...
Mevcut 16 iterasyonun, Iterasyon 17A–17E audit, Iterasyon 18A–18C veri koruma, Iterasyon 19A–19G maker-checker ve Iterasyon 20A–20C LDAP/oturum güvenliği dikeylerinin çekirdeği korunur. Banka kararları tamamlanmadan yeni HTTP yüzeyi açılmaz.
...
2. İterasyon 19H — Kontrollü kaynak pasifleştirme ve yeni iş kabulünü engelleme; sıradaki teknik aday.
...
| 19 | Maker-checker | `RULE-001`, `RULE-005`, `RULE-007`, `BFR-SOD-001`–`BFR-SOD-004` | Hazırlayan kendi kritik değişikliğini aktive edemez | 19A kritik kural, 19B scoring, 19C kural geri çekme, 19D kural süre aşımı, 19E veri kaynağı aktivasyonu, 19F kaynak onay geri çekme/süre aşımı ve 19G bağlantı revizyonu `TechnicallyVerified`; kontrollü pasife alma ve banka rol eşlemesi açık |
...
Sonraki teknik aday **İterasyon 19H — Kontrollü kaynak pasifleştirme** dilimidir.
Kapsam, güvenilir ve yetkili aktörün gerekçeli pasifleştirme işlemi, pasif kaynağın
yeni execution/schedule işi kabul etmemesi, mevcut işlerin politika dışına
taşmadan korunması ve durum geçişinin atomik audit edilmesiyle sınırlı
tutulmalıdır. Arşivleme ayrı artım kalmalıdır. Banka onaylı rol kodları ve gerçek
LDAP eşlemesi `ComplianceReviewRequired` kalır; 29D, HTTP/frontend ve hassas dışa
aktarma kendi açık banka/altyapı kararları tamamlanmadan uygulanmamalıdır.
