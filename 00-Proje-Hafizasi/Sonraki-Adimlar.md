...
Kesinleşen kararların uygulama backlogunda sürümlü `SourceUsagePolicy` kayıt ve
kota çözümleme dilimi 31A ile tamamlandı. Kalan davranışlar; kaynak çalışma
penceresi/yoğun saat kararı, CPU/IO ve hız sınırı gözlemi, dinamik timeout/retry,
...
Sıradaki hazır artım **İterasyon 31B — Kaynak çalışma penceresi ve yoğun saat
kararının fail-closed uygulanması**dır. CPU/IO ile hız sınırı için ölçüm adaptörü,
dinamik timeout/retry için yürütücü entegrasyonu ayrı dilimlerde kalmalıdır.
İterasyon 27B `OPEN-BNK-011/012`, gerçek fiziksel retention adaptörleri
`OPEN-BNK-008`, 21B/frontend `OPEN-BNK-020`, hassas dışa aktarma `OPEN-BNK-014`
ve gerçek SIEM/SOC `OPEN-BNK-010` kararlarını beklemektedir.
